...
const Profile = ({ navigation }) => {
  ...
  const setInfo = async () => {
    const email = await Keychain.getItemAsync('email');
    const nombre = await Keychain.getItemAsync('nombre');
    const apellido_paterno = await Keychain.getItemAsync('apellido_paterno');
    const apellido_materno = await Keychain.getItemAsync('apellido_materno');
    const matricula = await Keychain.getItemAsync('matricula');
    ...
    <>
      ...
      <View style={{ backgroundColor: "#FFCC00", flex: 1, padding: 18 }}></View>
        <View style={styles.containerImg}>
          <Image
            style={styles.img}
            source={require('../../appMovilRepo/assets/iconos/perfil.png')}></Image>
        </View>
